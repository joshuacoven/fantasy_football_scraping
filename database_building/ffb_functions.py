...
def ADP_assembly(start_year, current_year):
    database = []
    for x in range(start_year, current_year):
        page = requests.get("https://fantasyfootballcalculator.com/adp/standard/12-team/all/%d" % x)
        soup = BeautifulSoup(page.content, 'html.parser')
        table = soup.find_all('table')[0]
        df = pd.read_html(str(table), header = 0)
        dfyear = df[0]
...
def newer_assembly(start_year, current_year, rk_opt):
    url = 'https://www72.myfantasyleague.com/%d/reports?R=ADP&POS=*&PERIOD=ALL&CUTOFF=5&INJURED=1&IS_PPR=2&IS_KEEPER=1&IS_MOCK=0&PAGE=ALL'
    database = []
    for x in range(start_year, current_year):
